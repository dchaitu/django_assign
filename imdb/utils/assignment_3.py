...
def get_movie_names_with_actor_name_ending_with_smith():
    ...
    return [movie.name for movie in Movie.objects.filter(actors__name__iendswith="Smith") ]
...
def get_movie_names_with_ratings_above_given_minimum():
    ...
    return [movie.name for movie in Movie.objects.filter(rating__in=ratings)]
...
def get_movies_by_given_movie_names(movie_names):
    try:
        ...
        for movie in movies:
            ...
            movie_obj.update(movie.get_movie_dict())
            movie_obj.update({"average_rating": round(average_rating,2),
        ...
        print("Rating is not created")

    except ZeroDivisionError:
        print("count of rating is zero")
